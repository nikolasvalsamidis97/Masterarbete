...
# User settings
# -----------------------------------------------------------------------------
TABLES_ROOT = pathlib.Path(__file__).resolve().parent / "results" / "Tables"
PLOT_ATOMS = True
PLOT_MOLECULES = True
...
SAVE_FIGURE = True
SHOW_FIGURE = False
OUTPUT_DIR = pathlib.Path(__file__).resolve().parent / "results" / "Plots" / "Beta_vs_tgas"
...
def format_temperature_title(teff_k: float | None) -> str:
    if teff_k is None:
        return "unknown"
    digits = f"{int(round(teff_k)):,}".replace(",", r"\,")
    return rf"${digits}\,\mathrm{{K}}$"
...
def build_atom_colormap() -> Tuple[LinearSegmentedColormap, Normalize]:
    ...
    red_high_position = float(norm(ATOM_BETA_RED_HIGH))
    green_low_position = float(norm(ATOM_BETA_GREEN_LOW))
    green_center_position = float(norm(1.0))
    ...
    cmap = LinearSegmentedColormap.from_list(
        ...
        [
            (0.0, "#7f0000"),
            (red_high_position, "#d73027"),
            (red_high_position, "#f6d743"),
            (green_low_position, "#f6d743"),
            (green_center_position, "#00a651"),
...
def plot_atom_triptych(
    ...
    triptych_row_height = ROW_HEIGHT * 1.05
    ...
    for ax, (_target_label, _path, metadata, x_values, y_matrix, species_labels, _) in zip(axes, datasets):
        ...
        ax.set_title(format_temperature_title(stellar_teff), fontsize=AXIS_LABEL_SIZE)
    ...
    fig.suptitle(r"$\beta$ heatmap vs $T_{\rm exc}$ | $N_{\rm col}=0$", fontsize=TITLE_SIZE, y=0.93)
    fig.subplots_adjust(right=0.885, top=0.885, bottom=0.075, wspace=0.08)
