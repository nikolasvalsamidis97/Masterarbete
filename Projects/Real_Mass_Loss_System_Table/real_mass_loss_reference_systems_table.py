...
COMPOSITION_SOURCE_KEYS = {
    ...
    "barkaoui2024extended",
    "yan2018extended",
    "hoeijmakers2018atomic",
    "hoeijmakers2019spectral",
}
BOTH_PARAMETER_AND_COMPOSITION_SOURCE_KEYS = {
    "xue2024jwst",
    "giacalone2022hd",
    "mancini2020highly",
}
...
def source_marker_for_system(system_key: str) -> str:
    ...
    if not source_keys:
        return ""
    marker = ",".join(source_number_text(source_key) for source_key in source_keys)
    return rf"\textsuperscript{{{marker}}}"


def source_number_text(source_key: str) -> str:
    number = str(SOURCE_INDEX[source_key])
    if source_key in BOTH_PARAMETER_AND_COMPOSITION_SOURCE_KEYS:
        return rf"\textcolor{{blue}}{{{number}}}"
    if source_key in COMPOSITION_SOURCE_KEYS:
        return rf"\textcolor{{red}}{{{number}}}"
    return number


def source_footnote_parts() -> list[str]:
    return [
        rf"\textsuperscript{{{source_number_text(source_key)}}}{reference_text}"
        for source_key, reference_text in SOURCE_REFERENCES
    ]
...
def build_table_tex() -> str:
    ...
    composition_note = (
        ...
        r"Red source numbers mark composition-basis references; blue source numbers mark references "
        r"used for both adopted parameters and composition."
    )
    lines = [
        r"\begin{landscape}",
        r"\scriptsize",
