...
# User settings
# -----------------------------------------------------------------------------
STUDY_ROOT = pathlib.Path(__file__).resolve().parent / "results" / "Plots" / "T_atm_vs_beta1"
PLANET_FOLDER_GLOB = "*_r_beta1"
FILE_GLOB = "*_r_beta1.txt"
OUTPUT_PDF = None  # None -> save in each planet folder as <planet>_T_atm_vs_beta1.pdf
TITLE_PREFIX = r"$T_{\rm atm}$ vs critical height"
FIGSIZE = (10, 6)
FONT_SIZE = 13
AXES_LABEL_SIZE = 14
TITLE_SIZE = 16
...
METADATA_BOX_AXES_POS = (0.035, 0.72)
SKIP_ALL_NAN_FILES = True

AUTO_LEGEND_IF_CROWDED = False
...
def safe_output_name(planet_name: str) -> str:
    return planet_name.replace(" ", "_")
...
def make_metadata_box_text(metadata: Dict[str, Any]) -> str:
    ...
    mass = metadata.get("planet_mass_Mjup")
    ...
    if isinstance(mass, (int, float)):
        lines.append(rf"$M = {mass:.2f}\ M_{{\rm jup}}$")
...
def choose_output_path(planet_folder: pathlib.Path, planet_name: str, output_pdf):
    ...
    return planet_folder / f"{safe_output_name(planet_name)}_T_atm_vs_beta1.pdf"
...
def plot_one_planet_folder(planet_folder: pathlib.Path) -> None:
    ...
    for table_file, metadata, x_values, y_values in entries:
        species = metadata.get("species", table_file.stem.replace("_r_beta1", "")).replace("_", " ")
    ...
    ax.set_xlabel(r"Atmospheric temperature, $T_{\rm atm}$ [K]", fontsize=AXES_LABEL_SIZE)
    ax.set_ylabel(r"Critical height, $r_{\beta=1}/R_p$", fontsize=AXES_LABEL_SIZE)
    ax.set_title(rf"Critical height vs $T_{{\rm atm}}$ | {planet_name}", fontsize=TITLE_SIZE)
