...
RUNTIME_CACHE_DIR = REPO_ROOT / "Tables" / "runtime_cache" / "test_molecule_temperature_array"
...
def output_path(label: str) -> pathlib.Path:
    return pathlib.Path(__file__).with_name(f"test_molecule_temperature_array_{label}.txt")
