...
vlim = 10 * u.km/u.s
Npts = 1000
Fe_broadening = BroadeningProfile(Fe, b , vlim, Npts, 'Voigt')
vsini = 13 * u.km / u.s
...
distance = 0.1 * u.au
Temp = np.linspace(100, 10000, 10) * u.K

Ncol = np.logspace(7, 20, 50) * u.cm**(-2)

Fe_Ph = PhotonPressure(Fe_broadening, star)

Fe_ph_calc, _, _, _ = Fe_Ph.calc_PhotonPressure(Ncol, Temp, distance, chunk_size=1)
