...
def save_total_grid_outputs(rows: List[dict], planet_keys: Iterable[str] | None = None) -> None:
    ...
    for planet_key in planets:
        planet_rows = [row for row in total_rows if row["planet"] == planet_key]
        ...
        txt_path = OUTPUT_DIR / f"{safe_name(planet_key)}_total_mass_loss_vs_teff_distance.txt"
        save_plotdata_txt(
            txt_path,
            dataset_name=f"{safe_name(planet_key)}_total_mass_loss_vs_teff_distance",
        ...
        print(f"Saved total mass-loss grid to {txt_path}")
...
def main() -> None:
    ...
    for planet_key, star_key, distance_au in iter_systems():
        ...
        # Persist progress after every completed star-distance system so an
        # interruption only loses the current in-flight system, not whole planets.
        if system_rows:
            if USE_CHECKPOINT:
