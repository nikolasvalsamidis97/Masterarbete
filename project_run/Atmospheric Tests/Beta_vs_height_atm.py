...
# None -> use all species present in each selected planet composition, after skip
# filters below.
...
def resolve_species_list(module, planet_key, composition_species):
    if SELECTED_SPECIES is None:
        requested_species = [
            species
            for species in composition_species
            if species_matches_run_filters(module, species) and species not in EXCLUDED_SPECIES
        ]
        print(
        ...
        requested_species = [
            ...
            if species in composition_species
            and species_matches_run_filters(module, species)
            and species not in EXCLUDED_SPECIES
        ]
        print(f"Using explicitly selected species for {planet_key}: {requested_species}")
...
def main():
    ...
    for planet_key in planet_keys:
        ...
        requested_species = resolve_species_list(module, planet_key, composition_species)
        ...
        if not used_species:
            print(f"Skipping save for {planet_key}: no species completed successfully.")
            continue
